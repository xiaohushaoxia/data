<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="80%" height="70%" @close="close">
    ...
    <el-table :data="statistic" border style="width: 100%">
      ...
      <el-table-column prop="id" label="协议号数量">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.account_total }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="博主总数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.blogger_total }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="协议号封号">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.account_baned }}
        </template>
      </el-table-column>
