<template >
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="90%" @close="close" >
    ...
    <el-table :data="statistic" border style="width: 100%">
      ...
      <el-table-column prop="id" label="协议号成功数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.account_success_total }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="协议号封号数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.ban_account_total  }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="预期分裂数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.predicted_generate }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="成功分裂数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.success_generate }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="分裂失败数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.failed_generate }}
        </template>
      </el-table-column>
