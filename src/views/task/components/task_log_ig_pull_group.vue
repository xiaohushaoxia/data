<template >
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="90%" @close="close" >
    ...
    <el-table :data="statistic" border style="width: 100%">
      <el-table-column prop="id" label="协议号总数量">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.account_total }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="协议号失败数量">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.account_fail_total }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="协议号成功数量">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.account_success_total }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="封号总数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.ban_account_total }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="粉丝总数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.users_total }}
        </template>
      </el-table-column>


      <el-table-column prop="id" label="尝试建群数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.try_create_group_total }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="拉群失败">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.create_group_failed_total }}
        </template>
      </el-table-column>
      <el-table-column prop="task_log_content.create_group_success_total" label="拉群成功">
      </el-table-column>
      <el-table-column prop="id" label="进群成功">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.add_member_success_total }}
        </template>
      </el-table-column>
      <el-table-column prop="task_log_content.add_member_failed_total" label="进群失败">
      </el-table-column>
      <!-- <el-table-column prop="id" label="目标拉群用户数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.user_total }}
        </template>
      </el-table-column> -->
      <el-table-column prop="task_log_content.success_user_total" label="发送到群">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.send_message_success_total }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="发送失败">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.send_message_failed_total }}
        </template>
      </el-table-column>
      <!-- <el-table-column prop="task_log_content.group_failed" label="拉群失败"> -->
      <!-- </el-table-column> -->
      <!-- <el-table-column prop="task_log_content.failed" label="发送失败">
      </el-table-column> -->
      <!-- <el-table-column prop="task_log_content.ban_account_total" label="协议号被封数">
      </el-table-column> -->
    </el-table>
    <br>
