<template>
  <el-dialog :title="title" :visible.sync="dialogFormVisible" width="80%" height="70%" @close="close">
    ...
    <el-table :data="statistic" border style="width: 100%">
      ...
      <el-table-column prop="id" label="协议号总数">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.total  }}
        </template>
      </el-table-column>
      <el-table-column prop="id" label="登录成功">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.success }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="封号">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.baned }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="网络问题">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.network_error }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="账号锁定">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.lock }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="ip封禁">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.ip_lock }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="帐号错误">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.expired }}
        </template>
      </el-table-column>

      <el-table-column prop="id" label="其他">
        <template slot-scope="scope">
          {{ scope.row.task_log_content.other }}
        </template>
      </el-table-column>

    </el-table>
    <br>
    <el-descriptions title="异常日志" direction="vertical" border>

    </el-descriptions>
    <el-table :data="list" border style="width: 100%">
      <el-table-column prop="task_log_content.username" label="协议号">
      </el-table-column>
      <el-table-column prop="task_log_content.group_name" label="分组号">
      </el-table-column>
      <el-table-column prop="task_log_content.result" label="详情">
      </el-table-column>
