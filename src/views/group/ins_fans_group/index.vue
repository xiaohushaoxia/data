<template>
  <div class="table-container">
    ...
    <el-table ref="tableSort" v-loading="listLoading" :data="list" :element-loading-text="elementLoadingText"
      ...
      <el-table-column label="操作" width="680px">
        <template #default="{ row }">
          <el-button type="primary" size="mini" @click="handleEdit(row)">编辑</el-button>
          <el-button type="success" @click="handleList(row)"> 查看详情</el-button>
          <el-button type="warning" @click="handleCollectionFollower(row)">粉丝采集 </el-button>
          <el-button type="primary" @click="handleCollectionLocation(row)"> 地区采集</el-button>
          <el-button type="success" @click="handleCollectionLanguage(row)">语言识别</el-button>
          <el-button type="danger" @click="handleGender(row)">性别识别</el-button>
          <el-button type="primary" native-type="submit" @click="handleExport(row)">导出</el-button> 
          <!-- <el-button type="success" @click="handleImport">导入粉丝</el-button> -->
    ...
    <CollectionLanguage ref="collectionLanguage" /> <!-- 语言识别-->
    <CollectionLocation ref="collectionLocation" /> <!-- 地区采集-->
    <Gender ref="gender" /> <!-- 性别识别-->
  </div>
</template>
...
import CollectionLanguage from './components/collectionLanguage.vue'    //语言识别
import CollectionLocation from './components/collectionLocation.vue'   //地区采集
import Gender from './components/gender.vue'   //性别识别
import UserLists from './components/lists.vue'
export default {
  ...
  components: {
    ...
    CollectionFollower,
    // CollectionFollowing,
    CollectionLanguage,   //语言识别
    CollectionLocation,   //地区采集
    UserLists,
    Gender,        //性别识别
  },
  filters: {
  ...
  methods: {
    //粉丝采集
    handleCollectionFollower(row) {
      this.$refs['collectionFollower'].handle(row)
    },
    //地区采集
    handleCollectionLocation(row) {
      console.log(row)
      this.$refs['collectionLocation'].handle(row)
    },
    //语言识别
    handleCollectionLanguage(row) {
      this.$refs['collectionLanguage'].handle(row)
    },
    //导出
    handleExport(row) {
    ...
    //性别识别
     handleGender(row) {
      this.$refs['gender'].showEdit(row)
    },
    //详情
    handleList(row) {
      // console.log(row)
      // this.$router.push({name:'instagram_users',query: {id:row.id}}) // 只能用 name
      // // this.$refs['lists'].showEdit(row)
      const { href }=this.$router.resolve({name:'instagram_users',query: {id:row.id}}) // 只能用 name
      // console.log(href)
