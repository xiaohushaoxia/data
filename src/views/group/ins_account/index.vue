<template>
  <div class="table-container">
    ...
    <el-table ref="tableSort" v-loading="listLoading" :data="list" :element-loading-text="elementLoadingText"
      ...
      <el-table-column align="left" min-width="30px" show-overflow-tooltip prop="total" label="统计" />
      ...
      <el-table-column label="操作" width="700px">
        <template #default="{ row }">
          <el-button  type="warning" native-type="submit" @click="handleRegister(row)">注册</el-button>
          <el-button type="primary" size="mini" @click="handleEdit(row)">编辑</el-button>
          <!-- <el-button type="danger" @click="handleGender(row)">性别识别</el-button> -->
          <el-button  type="success" native-type="submit" @click="handleExport(row)">导出</el-button>
          <el-button  type="warning" native-type="submit" @click="handleCheckAll(row)">批量检测</el-button>
          <el-button type="primary" @click="handleImport(row)">上传</el-button>
          <el-button  type="danger" @click="handleClear(row)"> 清理封号</el-button>
          <el-button  type="success" @click="handleList(row)"> 详情</el-button>
...
import AccountLists from './components/lists.vue'
export default {
  ...
  methods: {
    ...
    handleRegister(row) {
      this.$refs['register'].showEdit(row)
    },
    handleList(row) {
      // console.log(row)
      // this.$router.push({name:'account',query: {id:row.id,group_type:row.group_type}}) // 只能用 name
      const { href }=this.$router.resolve({name:'account',query: {id:row.id,group_type:row.group_type}}) // 只能用 name
      // console.log(href)
    ...
    async fetchData() {
      ...
      if (this.queryForm.group_types.length == 0) {
        this.queryForm.group_type = [6, 7, 8, 9, 10, 11,13]
      }
